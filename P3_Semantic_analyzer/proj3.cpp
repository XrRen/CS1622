...
{
  int sys = getIndex("system");
  if (sys != -1)
  {
    int sys_idx = InsertEntry(sys, CLASS, 0);
    SetAttr(sys_idx, PREDE_ATTR, true);

    OpenBlock();
    int readIn = getIndex("readln");
    if (readIn != -1)
    {
      int read_idx = InsertEntry(readIn, FUNC, 0);
      SetAttr(read_idx, PREDE_ATTR, true);
      SetAttr(read_idx, ARGNUM_ATTR, 1);
    }
    int printLn = getIndex("println");
    if (printLn != -1)
    {
      int print_idx = InsertEntry(printLn, FUNC, 0);
      SetAttr(print_idx, PREDE_ATTR, true);
      SetAttr(print_idx, ARGNUM_ATTR, 1);
    }
    CloseBlock();
  }
}
